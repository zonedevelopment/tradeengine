...
function detectPressureBlock(candles = [], side = "BUY") {
    const sample = Array.isArray(candles) ? candles.slice(-5) : [];
    if (sample.length < 4) {
        return {
            bullishBlock: false,
            bearishBlock: false,
            reversalConfirmedForBuy: false,
            reversalConfirmedForSell: false,
        };
    }

    const c1 = getCandle(sample[sample.length - 1]);
    const c2 = getCandle(sample[sample.length - 2]);
    const c3 = getCandle(sample[sample.length - 3]);
    const c4 = getCandle(sample[sample.length - 4]);
    const avgB = avgBody(sample, 5) || 1;

    const bullishCount = [c1, c2, c3, c4].filter((c) => c.isBull).length;
    const bearishCount = [c1, c2, c3, c4].filter((c) => c.isBear).length;

    const bullishImpulse =
        bullishCount >= 3 &&
        (c1.isBull || c2.isBull) &&
        (c1.body >= avgB * 0.9 || c2.body >= avgB * 0.9) &&
        c1.close >= c2.close &&
        c2.close >= Math.min(c3.close, c4.close);
    ...
    const bearishImpulse =
        ...
        (c1.body >= avgB * 0.9 || c2.body >= avgB * 0.9) &&
        c1.close <= c2.close &&
        c2.close <= Math.max(c3.close, c4.close);

    const reversalConfirmedForSell =
        c1.isBear &&
        (c1.close < c2.low || c1.close < Math.min(c2.close, c3.close));

    const reversalConfirmedForBuy =
        c1.isBull &&
        (c1.close > c2.high || c1.close > Math.max(c2.close, c3.close));

    return {
        bullishBlock: bullishImpulse && !reversalConfirmedForSell,
        bearishBlock: bearishImpulse && !reversalConfirmedForBuy,
        reversalConfirmedForBuy,
        reversalConfirmedForSell,
    };
}
...
function buildHierarchicalContext(candles = []) {
    ...
    const pressure = detectPressureBlock(candles);
...
function scoreHierarchyForSide(side, hc) {
    ...
    if (side === "SELL" && hc.pressure?.bullishBlock) {
        score -= 20;
        reasons.push("SELL_INTO_BULLISH_BLOCK");
    }

    if (side === "BUY" && hc.pressure?.bearishBlock) {
        score -= 20;
        reasons.push("BUY_INTO_BEARISH_BLOCK");
    }
...
function calculateEntryTimingScore(candles = [], side = "BUY", hc = null) {
    ...
    if (side === "BUY") {
        ...
        if (hc?.swingZone10?.nearTop) score -= 8;
        if (hc?.pressure?.bearishBlock) score -= 10;
        if (!hc?.pressure?.reversalConfirmedForBuy && hc?.swingZone10?.nearBottom) {
            score -= 6;
        }
    } else {
        ...
        if (c1.body >= avgB * 1.1) score += 4;
        if (hc?.swingZone10?.nearBottom) score -= 8;
        if (hc?.pressure?.bullishBlock) score -= 10;
        if (!hc?.pressure?.reversalConfirmedForSell && hc?.swingZone10?.nearTop) {
            score -= 6;
        }
    }
...
function calculateSignalScores({ candles = [], spreadPoints = 0, trendContext = null }) {
    ...
    return {
        ...
        breakdown: {
            ...
            buyChasePenalty,
            sellChasePenalty,
            pressure: hierarchical.pressure,
        },
    };
}
