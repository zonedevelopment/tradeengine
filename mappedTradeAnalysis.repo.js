...
async function insertMappedTradeAnalysis(data) {
    ...
    const sql = `
      INSERT INTO mapped_trade_analysis (
        firebase_user_id, account_id, event_time, symbol, pattern_type, trigger_pattern,
        mode, tick_volume, micro_trend, volume_profile, pre_pattern_shape, range_state,
        session_name, open_price, close_price, sl_price, tp_price, sl_pips, tp_pips,
        rr_ratio, profit, result, side, post_mortem, context_hash
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
