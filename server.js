...
const symbolConfig = {
  NORMAL: {
    "XAUUSD": { maxSpread: 100, pipMultiplier: 100, minSL: 550, maxSL: 800, minTP: 950, maxTP: 1200 },
    "XAUUSDm": { maxSpread: 100, pipMultiplier: 100, minSL: 550, maxSL: 800, minTP: 950, maxTP: 1200 },
    "XAUUSDM": { maxSpread: 100, pipMultiplier: 100, minSL: 550, maxSL: 800, minTP: 950, maxTP: 1200 },
    "XAUUSDc": { maxSpread: 100, pipMultiplier: 100, minSL: 550, maxSL: 800, minTP: 950, maxTP: 1200 },
    "XAUUSDC": { maxSpread: 100, pipMultiplier: 100, minSL: 550, maxSL: 800, minTP: 950, maxTP: 1200 },
    "BTCUSD": { maxSpread: 200, pipMultiplier: 100, minSL: 850, maxSL: 1150, minTP: 1150, maxTP: 1450 },
    "BTCUSDm": { maxSpread: 200, pipMultiplier: 100, minSL: 850, maxSL: 1150, minTP: 1150, maxTP: 1450 },
    "BTCUSDM": { maxSpread: 200, pipMultiplier: 100, minSL: 850, maxSL: 1150, minTP: 1150, maxTP: 1450 },
    "DEFAULT": { maxSpread: 30, pipMultiplier: 100, minSL: 50, maxSL: 1950, minTP: 100, maxTP: 3950 }
  },
  SCALP: {
    "XAUUSD": { maxSpread: 50, pipMultiplier: 100, minSL: 370, maxSL: 430, minTP: 550, maxTP: 630 },
    "XAUUSDm": { maxSpread: 50, pipMultiplier: 100, minSL: 370, maxSL: 430, minTP: 550, maxTP: 630 },
    "XAUUSDM": { maxSpread: 50, pipMultiplier: 100, minSL: 370, maxSL: 430, minTP: 550, maxTP: 630 },
    "XAUUSDc": { maxSpread: 50, pipMultiplier: 100, minSL: 370, maxSL: 430, minTP: 550, maxTP: 630 },
    "XAUUSDC": { maxSpread: 50, pipMultiplier: 100, minSL: 370, maxSL: 430, minTP: 550, maxTP: 630 },
    "BTCUSD": { maxSpread: 80, pipMultiplier: 100, minSL: 750, maxSL: 950, minTP: 800, maxTP: 1200 },
    "BTCUSDm": { maxSpread: 80, pipMultiplier: 100, minSL: 750, maxSL: 950, minTP: 800, maxTP: 1200 },
    "BTCUSDM": { maxSpread: 80, pipMultiplier: 100, minSL: 750, maxSL: 950, minTP: 800, maxTP: 1200 },
    "BTCUSDc": { maxSpread: 80, pipMultiplier: 100, minSL: 750, maxSL: 950, minTP: 800, maxTP: 1200 },
    "BTCUSDC": { maxSpread: 80, pipMultiplier: 100, minSL: 750, maxSL: 950, minTP: 800, maxTP: 1200 },
    "DEFAULT": { maxSpread: 20, pipMultiplier: 100, minSL: 250, maxSL: 950, minTP: 550, maxTP: 1950 }
  },
  MICRO_SCALP: {
    "XAUUSD": { maxSpread: 35, pipMultiplier: 100, minSL: 70, maxSL: 150, minTP: 100, maxTP: 250 },
    "XAUUSDm": { maxSpread: 35, pipMultiplier: 100, minSL: 70, maxSL: 150, minTP: 100, maxTP: 250 },
    "XAUUSDM": { maxSpread: 35, pipMultiplier: 100, minSL: 70, maxSL: 150, minTP: 100, maxTP: 250 },
    "XAUUSDc": { maxSpread: 35, pipMultiplier: 100, minSL: 70, maxSL: 150, minTP: 100, maxTP: 250 },
    "XAUUSDC": { maxSpread: 35, pipMultiplier: 100, minSL: 70, maxSL: 150, minTP: 100, maxTP: 250 },
    "BTCUSD": { maxSpread: 60, pipMultiplier: 100, minSL: 130, maxSL: 270, minTP: 50, maxTP: 150 },
    "BTCUSDm": { maxSpread: 60, pipMultiplier: 100, minSL: 130, maxSL: 270, minTP: 50, maxTP: 150 },
    "BTCUSDM": { maxSpread: 60, pipMultiplier: 100, minSL: 130, maxSL: 270, minTP: 50, maxTP: 150 },
    "DEFAULT": { maxSpread: 20, pipMultiplier: 100, minSL: 50, maxSL: 200, minTP: 50, maxTP: 150 }
  },
};
...
function getBodyHigh(candle = {}) {
  return Math.max(toNum(candle.open), toNum(candle.close));
}

function getBodyLow(candle = {}) {
  return Math.min(toNum(candle.open), toNum(candle.close));
}

function getBodyMid(candle = {}) {
  return (getBodyHigh(candle) + getBodyLow(candle)) / 2;
}
...
function analyzeSignalRefreshBodyFlow({
  ...
  if (side === "BUY") {
    ...
    supportiveProgression = Boolean(
      lastBodyLow >= prevBodyMid &&
      last.close >= Number(prev.close || 0) &&
      (
        lastBodyHigh > prevBodyHigh ||
        (prev2 && prevBodyLow >= prev2BodyLow && prevBodyHigh >= prev2BodyHigh)
      )
    );

    breakoutHoldByBody = Boolean(
      breakoutLevel > 0 &&
      last.close >= breakoutLevel &&
    ...
    followThroughConfirmed = Boolean(
      isBullish(last) &&
      lastBody >= avgBody * 0.85 &&
      (
        last.close > prevBodyHigh ||
        breakoutHoldByBody ||
        (breakoutLevel > 0 && last.close >= breakoutLevel)
      )
    );

    pullbackHolding = Boolean(
      isBearish(last) &&
      lastBody > 0 &&
      lastBody <= Math.max(avgBody * 1.05, prevBody * 1.08) &&
      !bodyTakeoverAgainstSide &&
      (
        last.close >= Math.max(prevBodyMid, breakoutLevel || 0, breakoutZoneLow || 0) ||
        liveReclaim
      )
    );
    ...
    bodyCloseSupportive = last.close <= prevBodyMid;
    supportiveProgression = Boolean(
      lastBodyHigh <= prevBodyMid &&
      last.close <= Number(prev.close || 0) &&
      (
        lastBodyLow < prevBodyLow ||
        (prev2 && prevBodyHigh <= prev2BodyHigh && prevBodyLow <= prev2BodyLow)
      )
    );
    ...
    pullbackHolding = Boolean(
      ...
      (
        last.close <= Math.min(
          prevBodyMid,
          breakoutLevel > 0 ? breakoutLevel : prevBodyMid,
          breakoutZoneHigh > 0 ? breakoutZoneHigh : prevBodyMid
        ) ||
        liveReclaim
      )
    );
...
function normalizePriceCandle(candle = {}) {
  return {
...
function detectSidewaysMarket({
  ...
  if (safeCandles.length < 6) {
    return empty;
  }

  const recent = safeCandles.slice(-6);
  const highs = recent.map((c) => Number(c.high || 0));
  const lows = recent.map((c) => Number(c.low || 0));
  const maxHigh = Math.max(...highs);
  const minLow = Math.min(...lows);
  const span = Math.max(0, maxHigh - minLow);

  if (!Number.isFinite(span) || span <= 0) {
    return empty;
  }

  const ranges = recent.map((c) => Math.max(0, Number(c.high || 0) - Number(c.low || 0)));
  const bodies = recent.map((c) => Math.abs(Number(c.close || 0) - Number(c.open || 0)));
  ...
  const displacement = Math.abs(
    Number(recent[recent.length - 1]?.close || 0) - Number(recent[0]?.open || 0)
  );
  const efficiency = span > 0 ? displacement / span : 0;

  const directions = recent
    .map(getCandleDirection)
  ...
  const alternationRatio =
    directions.length >= 2 ? flipCount / (directions.length - 1) : 0;
  ...
  for (let i = 1; i < recent.length; i++) {
    ...
    const curr = recent[i];
    if (
      Number(curr.high || 0) <= Number(prev.high || 0) &&
      Number(curr.low || 0) >= Number(prev.low || 0)
    ) {
      insideCount += 1;
    }
  }
  ...
  const compressionRatio =
    avgRange > 0 ? span / Math.max(avgRange * recent.length, 0.00001) : 1;

  const htfDirection = String(
    higherTfContext?.overallDirection ||
    pattern?.higherTfContext?.overallDirection ||
    "NEUTRAL"
  ).toUpperCase();
  const htfStrength = String(
    higherTfContext?.strength ||
    pattern?.higherTfContext?.strength ||
    "WEAK"
  ).toUpperCase();

  const breakoutRetest = pattern?.breakoutRetest || {};
  const breakoutActive =
    Boolean(pattern?.breakoutDetected) ||
    Boolean(pattern?.freshBreakout) ||
    Boolean(breakoutRetest?.breakoutDetected) ||
    Boolean(breakoutRetest?.retestAccepted);

  const trendFollow4 = pattern?.trendFollow4 || {};
  const strongFollowThrough =
    String(trendFollow4?.direction || "NEUTRAL").toUpperCase() !== "NEUTRAL" &&
    (
      String(trendFollow4?.strength || "WEAK").toUpperCase() === "STRONG" ||
      Boolean(trendFollow4?.volumeConfirmed)
    ) &&
    efficiency >= 0.48 &&
    bodyToRange >= 0.44;
  ...
  if (efficiency <= 0.35) {
    score += 1.4;
    reasonCodes.push("LOW_DIRECTIONAL_EFFICIENCY");
  }

  if (bodyToRange <= 0.38) {
    score += 1.1;
    reasonCodes.push("SMALL_REAL_BODIES");
  }

  if (alternationRatio >= 0.55) {
    score += 1.1;
    reasonCodes.push("HIGH_DIRECTION_FLIP");
  }

  if (insideCount >= 2) {
    score += 0.9;
    reasonCodes.push("MULTIPLE_INSIDE_BARS");
  }

  if (compressionRatio <= 0.62) {
    score += 0.9;
    reasonCodes.push("PRICE_COMPRESSION");
  }

  if (Boolean(pattern?.isVolumeDrying)) {
    score += 0.45;
    reasonCodes.push("VOLUME_DRYING");
  }

  if (htfDirection === "NEUTRAL" || htfStrength === "WEAK") {
    score += 0.55;
    reasonCodes.push("HTF_NOT_TRENDING");
  }

  if (!breakoutActive) {
    score += 0.35;
    reasonCodes.push("NO_BREAKOUT_CONFIRMATION");
  }

  if (strongFollowThrough) {
    score -= 1.6;
    reasonCodes.push("STRONG_FOLLOW_THROUGH_PRESENT");
  }

  if (breakoutActive && Math.abs(Number(pattern?.score || 0)) >= 2.6) {
    score -= 1.25;
    reasonCodes.push("HIGH_SCORE_BREAKOUT_CONTEXT");
  }

  const roundedScore = Number(score.toFixed(2));
  const active = roundedScore >= 3.2;
  const severity = roundedScore >= 4.4 ? "STRONG" : active ? "MODERATE" : "NONE";

  return {
    active,
    severity,
    score: roundedScore,
    suggestedMode: active ? "SCALP" : null,
    reasonCodes,
    metrics: {
      span: Number(span.toFixed(5)),
      avgRange: Number(avgRange.toFixed(5)),
      avgBody: Number(avgBody.toFixed(5)),
      bodyToRange: Number(bodyToRange.toFixed(4)),
      displacement: Number(displacement.toFixed(5)),
      efficiency: Number(efficiency.toFixed(4)),
      alternationRatio: Number(alternationRatio.toFixed(4)),
      flipCount,
      insideCount,
...
function shouldBlockPrimaryDecisionInSideways(decisionValue = "", sidewaysContext = null) {
  if (!sidewaysContext?.active) return false;

  const upper = String(decisionValue || "").trim().toUpperCase();
  if (!upper.startsWith("ALLOW_")) return false;
  if (upper.includes("MICRO_SCALP")) return false;
...
async function handleSignalCore(req, { isRefresh = false } = {}) {
  ...
  try {
    ...
    const basePattern = await analyzePattern({
    ...
    const sidewaysContext = detectSidewaysMarket({
      candles,
      pattern: basePattern,
    ...
    let finalDecisionReason = finalDecisionResult.reason || null;
    ...
    if (shouldBlockPrimaryDecisionInSideways(finalDecision, sidewaysContext)) {
      finalDecision = "NO_TRADE";
      finalDecisionReason =
        sidewaysContext?.severity === "STRONG"
          ? "SIDEWAYS_MARKET_STRONG"
...
function evaluateSignalRefreshValidation({
  ...
  if (side === "BUY") {
    ...
    if (breakoutState.retestRejected) {
      ...
      } else if (bodyFlow?.pullbackHolding) {
        waiting = true;
        reasons.push("WAIT_BODY_PULLBACK_CONFIRMATION");
      } else if (bodyFlow?.compression) {
        waiting = true;
        reasons.push("WAIT_BODY_COMPRESSION_CONFIRMATION");
      ...
      if (validated) {
        reasons.push(
          bodyFlow?.supportiveProgression
            ? "BODY_NON_BREAKOUT_BUY_PROGRESSIVE"
            : "NON_BREAKOUT_BUY_STRUCTURE_OK"
        );
      } else if (bodyFlow?.compression || bodyFlow?.pullbackHolding) {
        waiting = true;
        reasons.push("WAIT_BODY_CONFIRMATION");
      }
    }
  } else if (side === "SELL") {
    strongFollowThrough = Boolean(
    ...
    if (breakoutState.retestRejected) {
      ...
      if (validated) {
        reasons.push(
          bodyFlow?.supportiveProgression
            ? "BODY_NON_BREAKOUT_SELL_PROGRESSIVE"
            : "NON_BREAKOUT_SELL_STRUCTURE_OK"
        );
      } else if (bodyFlow?.compression || bodyFlow?.pullbackHolding) {
        waiting = true;
        reasons.push("WAIT_BODY_CONFIRMATION");
...
function evaluateSignalRefreshEntryPressure({
  ...
  if (Array.isArray(bodyFlow?.evidence) && bodyFlow.evidence.length > 0) {
    evidence.push(...bodyFlow.evidence);
  }

  evidence.splice(0, evidence.length, ...Array.from(new Set(evidence.filter(Boolean))));
...
app.post("/signal-refresh", async (req, res) => {
  try {
    ...
    const bodyFlow = analyzeSignalRefreshBodyFlow({
      pendingSide: responseSide || pendingSide,
...
app.post("/check-exit-signal", async (req, res) => {
  try {
    ...
    const result = await analyzeEarlyExit({
      ...
      candles,
      candlesM1: Array.isArray(candles_m1) ? candles_m1 : [],
      candlesM15: Array.isArray(candles_m15) ? candles_m15 : [],
      candlesM30: Array.isArray(candles_m30) ? candles_m30 : [],
      candlesH1: Array.isArray(candles_h1) ? candles_h1 : [],
      candlesH4: Array.isArray(candles_h4) ? candles_h4 : [],
      mode: resolvedMode,
