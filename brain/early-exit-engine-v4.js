...
function getExitProfile(mode = "NORMAL") {
    ...
    if (normalized === "MICRO_SCALP") {
        return {
            minProfitToProtect: 0.14,
            minProfitForHighRiskExit: 0.26,
            minProfitForStrongReversalExit: 0.34,
            moveToBeMinR: 0.46,
            moveToBeMinProgressToTarget: 0.28,
            strongReversalThresholdProfit: 3.4,
            hardCutLossReversalThreshold: 2.4,
            mediumBounceThreshold: 1.7,
            scalpTimeoutLowVolumeMinutes: 8,
            scalpTimeoutNoProgressMinutes: 6,
            smallProfitMaxProgressToTarget: 0.44,
            smallProfitMinPullbackFromPeak: 0.28,
            scalpMaxHoldingBars: 2,
            scalpMinExpectedProgressToTarget: 0.22,
            scalpStructureFailureLoss: -0.08,
            scalpFailedContinuationLoss: -0.06,
            earlyCutMinMinutes: 2,
            earlyCutMaxSlUsage: 0.22,
            timeoutCutMaxSlUsage: 0.30,
        };
    }

    if (normalized === "SCALP") {
        return {
            minProfitToProtect: 0.24,
            minProfitForHighRiskExit: 0.50,
            minProfitForStrongReversalExit: 0.68,
            moveToBeMinR: 0.62,
            moveToBeMinProgressToTarget: 0.32,
            strongReversalThresholdProfit: 3.7,
            hardCutLossReversalThreshold: 2.8,
            mediumBounceThreshold: 2.0,
            scalpTimeoutLowVolumeMinutes: 11,
            scalpTimeoutNoProgressMinutes: 8,
            smallProfitMaxProgressToTarget: 0.48,
            smallProfitMinPullbackFromPeak: 0.26,
            scalpMaxHoldingBars: 3,
            scalpMinExpectedProgressToTarget: 0.24,
            scalpStructureFailureLoss: -0.12,
            scalpFailedContinuationLoss: -0.09,
            earlyCutMinMinutes: 3,
            earlyCutMaxSlUsage: 0.28,
            timeoutCutMaxSlUsage: 0.36,
        };
    }
    ...
    return {
        ...
        hardCutLossReversalThreshold: 4.2,
        mediumBounceThreshold: 3.0,
        scalpTimeoutLowVolumeMinutes: 30,
        scalpTimeoutNoProgressMinutes: 18,
        ...
        scalpFailedContinuationLoss: -0.28,
        earlyCutMinMinutes: 8,
        earlyCutMaxSlUsage: 0.40,
        timeoutCutMaxSlUsage: 0.50,
    };
}
...
function shouldExitScalpTimeout({
    ...
    const lossUsageRatio = getLossUsageRatio(openPosition, profit, slPoints);

    const strongConfirm =
        confirmation.level === "STRONG" ||
        confirmation.hasStructureBreak;

    const mediumOrStrongConfirm =
    ...
    if (
        ...
        mins >= Math.max(5, profile.scalpTimeoutLowVolumeMinutes - 1) &&
        progress.progressToTarget >= 0.58 &&
        profitRetractionRatio >= Math.max(0.24, profile.smallProfitMinPullbackFromPeak) &&
        mediumOrStrongConfirm
    ) {
        return {
    ...
    if (
        ...
        mins >= profile.scalpTimeoutLowVolumeMinutes + 1 &&
        profit >= profile.minProfitToProtect &&
    ...
    if (
        mins >= profile.scalpTimeoutNoProgressMinutes + 2 &&
        profit >= profile.minProfitToProtect &&
    ...
    if (
        mins >= profile.earlyCutMinMinutes &&
        profit < 0 &&
        (
            lossUsageRatio >= profile.earlyCutMaxSlUsage ||
    ...
    if (
        ...
        profit < 0 &&
        lossUsageRatio >= profile.timeoutCutMaxSlUsage &&
    ...
    if (
        profit < 0 &&
        lossUsageRatio >= Math.max(0.45, profile.timeoutCutMaxSlUsage + 0.05) &&
        (
            confirmation.hasStructureBreak ||
            confirmation.level === "STRONG"
        )
        ...
        return {
            action: "CUT_LOSS_NOW",
            reason: "SCALP_HARD_DEFENSIVE_CUT",
        };
    }
...
function shouldCutWeakScalpTrade({
    ...
    if (mins < profile.earlyCutMinMinutes) return false;
    ...
    const fastLossReached =
        lossUsageRatio >= profile.earlyCutMaxSlUsage ||
        profit <= profile.scalpFailedContinuationLoss;

    return fastLossReached && weakMomentum && reversalScore >= 0.8 && mediumConfirm;
}
...
async function analyzeEarlyExit({
    ...
    if (normalizedMode === "SCALP" || normalizedMode === "MICRO_SCALP") {
        ...
        if (
            ...
            approxBarsHeld >= Math.max(2, profile.scalpMaxHoldingBars - 1)
        ) {
            return {
                action: "CUT_LOSS_NOW",
    ...
    if (profit < 0) {
        if (riskLevel === "CRITICAL" && adjustedScore >= 2.0 && confirmation.level !== "NONE") {
            return {
                action: "CUT_LOSS_NOW",
        ...
        if (riskLevel === "HIGH" && adjustedScore >= 2.8 && confirmation.level !== "NONE") {
            return {
                action: "CUT_LOSS_NOW",
                reason: "High risk + reversal confirmed",
                riskLevel,
                score: adjustedScore,
            };
        }
